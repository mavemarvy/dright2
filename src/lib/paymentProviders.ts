...
export const FALLBACK_PROVIDERS: PaymentProvider[] = [
  {
    id: 'fb-paystack', slug: 'paystack', name: 'Paystack', logo: '',
  ...
  {
    id: 'fb-wise', slug: 'wise', name: 'Wise (TransferWise)', logo: '',
    description: 'Low-cost international transfers with the real exchange rate.',
    status: 'coming_soon', priority: 6,
...
const PROVIDER_LOGOS: Record<string, string> = {
  paystack: '🟢', google_pay: '🔵', apple_pay: '⚪',
  flutterwave: '🟠', stripe: '🟣', wise: '🟩',
};
...
export async function fetchPaymentProviders(): Promise<PaymentProvider[]> {
  ...
  if (error || !data || data.length === 0) return FALLBACK_PROVIDERS;
  return data as PaymentProvider[];
}

export async function updateProviderConfiguration(
  providerId: string,
  updates: { status?: ProviderStatus; priority?: number; isRecommended?: boolean },
): Promise<{ success: boolean; error?: string }> {
  if (providerId.startsWith('fb-')) {
    return { success: false, error: 'Fallback providers cannot be modified until the database provider record exists.' };
  }

  const { error } = await supabase.rpc('admin_update_payment_provider', {
    p_provider_id: providerId,
    p_status: updates.status ?? null,
    p_priority: updates.priority ?? null,
    p_is_recommended: updates.isRecommended ?? null,
  });
