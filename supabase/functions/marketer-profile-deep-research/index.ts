...
const allowedHosts = new Set([
  "tiktok.com", "linkedin.com", "fiverr.com", "upwork.com", "youtube.com", "youtu.be",
  "instagram.com", "facebook.com", "x.com", "twitter.com", "github.com",
]);
...
async function serperSearch(q: string): Promise<{ results: SearchResult[]; error?: { code: string; message: string } }> {
  ...
  try {
    ...
    const results = Array.isArray(payload?.organic)
      ? payload.organic.map((item: any) => ({
          title: String(item.title || "").slice(0, 300),
          link: String(item.link || ""),
          snippet: String(item.snippet || "").slice(0, 800),
          source: "Serper/Google",
          query: q,
        })).filter((item: SearchResult) => validUrl(item.link))
      : [];
...
function safeParse(content: string) {
  ...
  try { return JSON.parse(clean); } catch {
    ...
    if (start >= 0 && end > start) {
      try { return JSON.parse(clean.slice(start, end + 1)); } catch { /* handled below */ }
    }
    return null;
  }
}
...
function normalizeReport(raw: any, evidence: any) {
  ...
  const classificationItems = (v: unknown) => arr(v).map((item) => ({ classification: evidenceStatus.includes(String((item as any)?.classification) as any) ? (item as any).classification : "UNKNOWN", detail: typeof item === "object" && item ? String((item as any).detail || "") : String(item) })).filter((x) => x.detail).slice(0, 30);
  ...
  return {
    ...
    identity_consistency: {
      status: status(raw?.identity_consistency?.status, ["consistent", "mixed", "unclear"], "unclear"),
      score: identityScore,
      evidence: arr(raw?.identity_consistency?.evidence),
    },
    profile_health: {
      score: profileHealthScore,
      status: profileHealthScore === null ? "unknown" : status(raw?.profile_health?.status, ["excellent", "good", "fair", "poor", "unknown"], "unknown"),
    },
    audience_authenticity: {
      score: authenticityScore,
      status: authenticityScore === null ? "unknown" : status(raw?.audience_authenticity?.status, ["high", "medium", "low", "unknown"], "unknown"),
      confidence: score(raw?.audience_authenticity?.confidence) ?? 0,
    },
    bot_risk: {
      score: botScore,
      status: botScore === null ? "unknown" : status(raw?.bot_risk?.status, ["low", "medium", "high", "unknown"], "unknown"),
      confidence: score(raw?.bot_risk?.confidence) ?? 0,
    },
    activity: {
      status: status(raw?.activity?.status, ["active", "inactive", "unknown"], "unknown"),
      evidence: arr(raw?.activity?.evidence),
    },
    platform_consistency: {
      status: status(raw?.platform_consistency?.status, ["consistent", "mixed", "unclear"], "unclear"),
    ...
    _evidence_meta: {
      ...
      source_domains: [...new Set(evidence.search_results.map((x: SearchResult) => { try { return new URL(x.link).hostname.replace(/^www\./, ""); } catch { return "unknown"; } }))].slice(0, 30),
    },
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (adminError || !admin?.is_admin || String(admin.admin_status || "").toLowerCase() !== "active" || !allowedRoles.has(String(admin.admin_role || ""))) {
      return json({ success: false, error: { code: "FORBIDDEN", message: "Sales Team administration permission required." } }, 403);
    }
    ...
    const { data: applicant, error: applicantError } = await client.from("users")
      .select("id,email,full_name,username,profession,social_media_links,marketer_social_analysis,marketer_deep_research")
    ...
    const submittedLinks = Array.isArray(applicant.social_media_links)
      ? applicant.social_media_links.map((v: unknown) => String(v).trim()).filter(validUrl)
      : [];
    ...
    for (const profileUrl of targets.slice(0, 10)) {
      ...
      const evidence = {
        ...
        existing_profile_analysis: {
          ...
          confidence_score: basic.confidence_score ?? null,
          signals: Array.isArray(basic.signals) ? basic.signals.slice(0, 20) : [],
          analyzed_at: basic.analyzed_at ?? null,
        },
      ...
      if (!normalized.length && serperError?.code === "SERPER_NOT_CONFIGURED") {
        errors.push({ profile_url: profileUrl, stage: "search", error: serperError });
        reports.push({ profile_url: profileUrl, platform, status: "failed", provider: "Serper/Google", model: null, research: null, requested_at: new Date().toISOString() });
        continue;
      }
...
- OBSERVED PUBLIC DATA: visible in a public search result/page but not independently verified.
- CALCULATED: mathematically derived only from supplied values.
- ESTIMATE: an assessment based on evidence; never present it as fact.
- UNKNOWN: cannot safely be established from supplied evidence.
...
Supplied evidence: ${JSON.stringify(evidence)}`;
      ...
      try {
        ...
        const aiResponse = await fetch(aiUrl, {
          ...
          body: JSON.stringify({ feature: "generate-text", prompt, useCache: false }),
        });
        const aiText = await aiResponse.text();
        let aiJson: any = null;
        try { aiJson = JSON.parse(aiText); } catch { /* handled below */ }
        ...
        research = normalizeReport({
          ...
          profile_health: { score: null, status: "unknown" },
          audience_authenticity: { score: null, status: "unknown", confidence: 0 },
          bot_risk: { score: null, status: "unknown", confidence: 0 },
          identity_consistency: { status: "unclear", score: null, evidence: [] },
          activity: { status: "unknown", evidence: [] },
          platform_consistency: { status: "unclear", evidence: [] },
          verified_facts: [], observed_data: [], calculated_metrics: [], unknowns: [message], limitations: ["Automated AI analysis did not complete."], recommendation: "manual_review", recommendation_reason: "Manual review is required because the AI analysis did not complete.",
        }, evidence);
      }
      ...
      if (reportStatus === "completed") {
        const { error: saveError } = await client.from("marketer_profile_deep_research").insert({
    ...
    const merged = [...successful, ...previousHistory].slice(0, 50);
    const { error: userUpdateError } = await client.from("users").update({ marketer_deep_research: merged, updated_at: new Date().toISOString() }).eq("id", applicantId);
    if (userUpdateError) {
      console.error("[marketer-profile-deep-research] users history save failure", userUpdateError.message);
      errors.push({ stage: "database_history", error: { code: "DATABASE_SAVE_ERROR", message: "Research history could not be updated." } });
    }

    return json({ success: successful.length > 0, reports, errors: errors.length ? errors : undefined });
  } catch (error) {
