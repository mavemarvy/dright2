...
async function workerSecret() {
  return sha256Hex(`dright-broadcast-worker-v1:${BOT_TOKEN}`);
}
async function telegram(method: string, payload: Record<string, unknown> = {}) {
  ...
  if (!response.ok || parsed?.ok !== true) throw new Error(`TELEGRAM_${Number(parsed?.error_code || response.status || 500)}`);
...
async function ensureKnownChat(chatId: string) {
...
function allowedPhotoUrl(raw: string) {
  try {
    ...
    return url.protocol === "https:" && (
      url.hostname === "vtiardblxpaeekbfvhjo.supabase.co" ||
      url.hostname === "dright.store" ||
      url.hostname === "www.dright.store"
    );
  } catch { return false; }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (action === "list_state") {
      const [chats, joins, campaigns, settings, subscribers] = await Promise.all([
        ...
        adminDb.from("telegram_broadcast_subscribers").select("telegram_user_id,is_active,subscribed_news,subscribed_promotions,subscribed_recommendations,created_at,last_seen_at").eq("is_active", true),
      ]);
      return json({
        success: true,
        chats: chats.data || [],
    ...
    if (action === "update_chat") {
      ...
      const { data, error } = await adminDb.from("telegram_broadcast_chats")
        ...
        .eq("chat_id", chatId).select("*").single();
    ...
    if (action === "set_title") {
      ...
      await adminDb.from("telegram_broadcast_chats").update({ title: clean(chat?.title,240) || title, updated_at: new Date().toISOString() }).eq("chat_id", chatId);
    ...
    if (action === "set_description") {
      ...
      await ensureKnownChat(chatId);
      await telegram("setChatDescription", { chat_id: chatId, description });
    ...
    if (action === "set_photo") {
      ...
      if (!photoResponse.ok) throw new Error("PHOTO_DOWNLOAD_FAILED");
      const contentType = clean(photoResponse.headers.get("content-type"),100).toLowerCase();
    ...
    if (action === "update_settings") {
      ...
      const { data, error } = await adminDb.from("telegram_broadcast_settings").update(patch).eq("singleton",true).select("*").single();
      if (error) throw error;
    ...
    if (action === "publish") {
      ...
      const subscriberAudience = sourceType === "news" ? "news" : sourceType === "recommendation" ? "recommendations" : "news";
      const { data: campaign, error } = await adminDb.from("telegram_broadcast_campaigns").insert({
        ...
        audience: {
          ...
          subscribers: input.publish_to_subscribers === false ? [] : [subscriberAudience],
        },
      ...
      return json({ success: true, campaign, worker });
    }
