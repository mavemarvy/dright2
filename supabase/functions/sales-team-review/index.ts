...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(req.method==='GET'){
      const {data:pending,error}=await client.from('users').select('id,email,full_name,username,avatar_url,created_at,joined_at,location,profession,marketer_level,marketer_status,social_media_links,verification_status,is_verified,last_active_at').eq('marketer_status','pending').order('created_at',{ascending:false});
      ...
      return json({pendingApplications:pending||[],reviewHistory:recent||[]});
    }
    if(req.method==='POST'){
      ...
      const {data:applicant,error:applicantError}=await client.from('users').select('id,email,full_name,marketer_level,marketer_status,social_media_links').eq('id',applicantId).single();
      ...
      const {data:updated,error:updateError}=await client.from('users').update(update).eq('id',applicantId).select('id,email,full_name,marketer_level,marketer_status,social_media_links').single();
