  ...
  if p_period<>'current' then
    ...
    simulated_base as (
      select
        ...
        jsonb_build_object(
          ...
          'increment_amount',coalesce(sc.increment_amount,0),
          'increment_interval_seconds',coalesce(sc.increment_interval_seconds,3600)
        ) as detail,
    ...
    unranked_zero as (
      select
        ...
        *,
        row_number() over(
          order by is_simulated asc, activity_at desc nulls last, entry_id
