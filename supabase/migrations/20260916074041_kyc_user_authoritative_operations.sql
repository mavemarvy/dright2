...
BEGIN
  ...
  INSERT INTO public.kyc_documents(submission_id,user_id,doc_type,doc_url,doc_name,doc_mime_type,doc_size_bytes,status,version,storage_bucket,storage_path,issuing_country,expires_at,created_by,updated_by)
  VALUES(p_submission_id,v_uid,p_doc_type,p_storage_path,p_original_name,p_mime_type,p_size_bytes,'pending',v_version,'kyc-docs',p_storage_path,CASE WHEN p_issuing_country IS NULL THEN NULL ELSE upper(p_issuing_country) END,p_expires_at,v_uid,v_uid) RETURNING * INTO v_row;
  IF v_old.id IS NOT NULL THEN UPDATE public.kyc_documents SET status='replaced',replaced_by=v_row.id,updated_by=v_uid,updated_at=now() WHERE id=v_old.id; END IF;
  INSERT INTO public.kyc_audit_logs(user_id,action,entity_type,entity_id,metadata)
  VALUES(v_uid,CASE WHEN v_old.id IS NULL THEN 'document_uploaded' ELSE 'document_replaced' END,'kyc_document',v_row.id,jsonb_build_object('doc_type',p_doc_type,'version',v_version,'replaced_document_id',v_old.id));
  RETURN v_row;
END;
$$;
...
FOR DELETE TO authenticated USING (bucket_id='kyc-docs' AND owner=auth.uid() AND NOT EXISTS (SELECT 1 FROM public.kyc_documents d WHERE d.storage_bucket='kyc-docs' AND d.storage_path=storage.objects.name AND d.is_deleted=false));