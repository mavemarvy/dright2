...
CREATE OR REPLACE FUNCTION public.register_kyc_document(
  p_submission_id uuid,p_doc_type text,p_storage_path text,p_original_name text,p_mime_type text,p_size_bytes bigint,
  p_issuing_country text DEFAULT NULL,p_expires_at timestamptz DEFAULT NULL,p_replaces_document_id uuid DEFAULT NULL
...
BEGIN
  ...
  VALUES(p_submission_id,v_uid,p_doc_type,p_storage_path,p_original_name,p_mime_type,p_size_bytes,'pending',v_version,'kyc-docs',p_storage_path,CASE WHEN p_issuing_country IS NULL THEN NULL ELSE upper(p_issuing_country) END,p_expires_at,v_uid,v_uid)
