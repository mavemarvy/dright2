CREATE OR REPLACE FUNCTION public.process_paystack_payment(p_reference text,p_user_id uuid,p_amount numeric,p_purpose text,p_reference_id uuid,p_metadata jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_tx public.paystack_transactions%ROWTYPE; v_currency text;
BEGIN
 ...
 IF p_purpose='sales_team_contract' THEN v_currency:=upper(v_tx.currency); RETURN public.process_verified_sales_team_contract_payment(p_reference,p_user_id,v_tx.amount,v_currency,'paystack'); END IF;
 RETURN public.process_paystack_payment_core_st6(p_reference,p_user_id,v_tx.amount,v_tx.purpose,v_tx.reference_id,COALESCE(v_tx.metadata,'{}'::jsonb));
END $$;
...
UPDATE public.paystack_transactions SET metadata=COALESCE(metadata,'{}'::jsonb)||jsonb_build_object('reconciliation_state','legacy_success_requires_gateway_reverification','reconciliation_flagged_at',now()),updated_at=now()
WHERE status='success' AND processed_at IS NULL AND created_at<timestamptz '2026-09-06 00:00:00+00' AND COALESCE(metadata->>'reconciliation_state','')='';

CREATE OR REPLACE FUNCTION public.get_financial_reconciliation_health() RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_role text:=COALESCE(auth.role(),''); v_uid uuid:=auth.uid(); v_is_admin boolean:=false; v_result jsonb;
BEGIN
 IF v_role<>'service_role' THEN IF v_uid IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF; SELECT COALESCE(u.is_admin,false) INTO v_is_admin FROM public.users u WHERE u.id=v_uid; IF NOT COALESCE(v_is_admin,false) THEN RAISE EXCEPTION 'Admin access required'; END IF; END IF;
 ...
 'wallets',jsonb_build_object(
  ...
  'ledger_latest_balance_mismatches',(SELECT count(*) FROM (SELECT DISTINCT ON(le.wallet_id,le.account)le.wallet_id,le.account,le.balance_after FROM public.ledger_entries le ORDER BY le.wallet_id,le.account,le.created_at DESC,le.id DESC)x JOIN public.cc_wallets w ON w.id=x.wallet_id WHERE abs(COALESCE(x.balance_after,0)-COALESCE(CASE x.account WHEN 'balance' THEN w.balance WHEN 'escrow_balance' THEN w.escrow_balance WHEN 'pending_balance' THEN w.pending_balance WHEN 'locked_balance' THEN w.locked_balance WHEN 'referral_balance' THEN w.referral_balance WHEN 'affiliate_balance' THEN w.affiliate_balance WHEN 'creator_balance' THEN w.creator_balance WHEN 'advertiser_budget' THEN w.advertiser_budget WHEN 'seller_earnings' THEN w.seller_earnings ELSE x.balance_after END,0))>0.01)),
