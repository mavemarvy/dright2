...
-- Backfill the one legacy/pending campaign created before ST-5A captured billing state.
-- The ST-5A guard intentionally blocks ordinary non-service writes, so the migration performs
-- this one-time canonical backfill with the guard trigger disabled inside this migration only.
...
BEGIN
  ...
  -- This adapter is intentionally provider-specific. Paystack verification occurs in the
  -- existing verify/webhook functions before this RPC; provider registry state is checked here.
