...
FOR EACH ROW EXECUTE FUNCTION public.guard_order_financial_attribution_snapshot();
...
BEFORE UPDATE ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.guard_order_client_conversion_state();
REVOKE ALL ON FUNCTION public.guard_order_client_conversion_state() FROM PUBLIC, anon, authenticated;
...
AS $function$
DECLARE
  o record; r record; v_amount numeric; v_recipient uuid; v_role text; v_source text;
  v_basis numeric; v_pool numeric; v_result jsonb; v_wallet_id uuid;
  v_account_status text; v_marketer_status text; v_marketer_level integer;
  v_advertiser_status text; v_advertiser_grade text; v_required_grade text;
  v_required_marketer_level integer;
BEGIN
  IF coalesce(auth.role(), '') <> 'service_role' THEN RAISE EXCEPTION 'Commission distribution requires service_role'; END IF;
  SELECT * INTO o FROM public.orders WHERE id=p_order_id FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success',false,'error','Order not found'); END IF;
  IF upper(coalesce(o.status,''))<>'COMPLETED' THEN RETURN jsonb_build_object('success',false,'error','Order is not completed'); END IF;
  ...
  IF v_source='affiliate' THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Affiliate payout handled by payment conversion engine'); END IF;
  ...
  FROM public.users WHERE id=v_recipient;
  IF NOT FOUND OR upper(coalesce(v_account_status,''))<>'ACTIVE' THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Attributed recipient is not active'); END IF;
  IF v_source='sales_team' THEN
    IF lower(coalesce(v_marketer_status,''))<>'approved' OR coalesce(v_marketer_level,0)<3 THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Sales Team recipient is not approved/eligible'); END IF;
    IF coalesce(o.source_level,'') ~* '^Mkt L[3-5]$' THEN
      v_required_marketer_level:=substring(o.source_level from '([3-5])$')::integer;
      IF v_marketer_level<>v_required_marketer_level THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Sales Team level no longer matches attribution'); END IF;
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM public.sales_team_members stm JOIN public.sales_teams st ON st.id=stm.sales_team_id
      WHERE stm.user_id=v_recipient AND lower(coalesce(stm.status,''))='active' AND lower(coalesce(st.status,''))='active'
        AND (o.sales_team_id IS NULL OR stm.sales_team_id=o.sales_team_id)
    ) THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Sales Team membership is no longer active'); END IF;
  ELSE
    IF lower(coalesce(v_advertiser_status,''))<>'approved' THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Advertiser recipient is not approved'); END IF;
    v_required_grade:=CASE v_source
      WHEN 'pro_advertiser' THEN 'Pro' WHEN 'super_advertiser' THEN 'Super' WHEN 'partnership' THEN 'Partnership'
      WHEN 'advertiser' THEN CASE
        WHEN coalesce(o.source_level,'') ~* '(Adv[[:space:]]+)?A$' THEN 'A'
        WHEN coalesce(o.source_level,'') ~* '(Adv[[:space:]]+)?B$' THEN 'B'
        WHEN coalesce(o.source_level,'') ~* '(Adv[[:space:]]+)?C$' THEN 'C' ELSE NULL END
      ELSE NULL END;
    IF v_required_grade IS NOT NULL AND lower(coalesce(v_advertiser_grade,''))<>lower(v_required_grade) THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Advertiser grade no longer matches attribution'); END IF;
    IF v_source='advertiser' AND v_required_grade IS NULL AND upper(coalesce(v_advertiser_grade,'')) NOT IN ('A','B','C') THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','Advertiser grade is not eligible for base advertiser attribution'); END IF;
  END IF;
  v_basis:=greatest(0,coalesce(o.final_price,0));
  v_pool:=greatest(0,coalesce(o.sales_team_task_amount,0));
  IF v_pool<=0 AND v_source IN ('sales_team','advertiser','pro_advertiser','super_advertiser','partnership') AND coalesce(o.admin_task_amount,0)<=0 THEN
    v_pool:=greatest(0,coalesce(o.final_price,0)-coalesce(o.base_price,0)-coalesce(o.tier_price,0)-coalesce(o.customization_price,0));
  END IF;
  IF v_basis<=0 OR v_pool<=0 THEN RETURN jsonb_build_object('success',true,'distributed',false,'reason','No attributed commission basis/pool'); END IF;
  IF EXISTS (SELECT 1 FROM public.commission_splits WHERE order_id=o.id AND source_type=v_source AND status='distributed') THEN RETURN jsonb_build_object('success',true,'distributed',true,'idempotent',true); END IF;
  ...
  ORDER BY (campaign_id IS NOT NULL) DESC,(product_id IS NOT NULL) DESC,(advertiser_grade IS NOT NULL) DESC,(source_level IS NOT NULL) DESC,priority ASC,created_at DESC LIMIT 1;
  ...
  IF v_wallet_id IS NULL THEN
    INSERT INTO public.cc_wallets(user_id) VALUES(v_recipient) ON CONFLICT (user_id) DO NOTHING RETURNING id INTO v_wallet_id;
    IF v_wallet_id IS NULL THEN SELECT id INTO v_wallet_id FROM public.cc_wallets WHERE user_id=v_recipient LIMIT 1 FOR UPDATE; END IF;
  END IF;
  ...
  VALUES(o.id,v_recipient,v_role,v_amount,r.percentage,'balance','distributed',now(),v_source,v_recipient,r.id,v_basis,
    jsonb_build_object('source_level',o.source_level,'referrer_id',o.referrer_id,'sales_team_id',o.sales_team_id,'team_member_id',o.team_member_id,'team_lead_id',o.team_lead_id,'tracking_code',o.tracking_code,'referral_link_id',o.referral_link_id,'campaign_id',o.campaign_id));
...
CREATE OR REPLACE FUNCTION public.process_paystack_payment_core_st6(
  p_reference text,p_user_id uuid,p_amount numeric,p_purpose text DEFAULT 'wallet_funding'::text,p_reference_id uuid DEFAULT NULL::uuid,p_metadata jsonb DEFAULT '{}'::jsonb
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_tx public.paystack_transactions%ROWTYPE; v_wallet_id uuid; v_result jsonb; v_order public.orders%ROWTYPE;
  v_seller_wallet_id uuid; v_affiliate_wallet_id uuid; v_sale_id uuid; v_commission numeric:=0;
  v_seller_earnings numeric:=0; v_order_final numeric:=0; v_effective_sales_task_amount numeric:=0;
  v_effective_affiliate_commission numeric:=0; v_attributed_split jsonb:='{}'::jsonb;
  v_referrer_account_status text; v_affiliate_eligible boolean:=false;
BEGIN
  IF coalesce(auth.role(),'')<>'service_role' THEN RAISE EXCEPTION 'Payment processing requires service_role'; END IF;
  ...
  IF v_tx.processed_at IS NOT NULL THEN RETURN jsonb_build_object('success',true,'message','Already processed','idempotent',true); END IF;
  SELECT id INTO v_wallet_id FROM public.cc_wallets WHERE user_id=p_user_id FOR UPDATE;
  IF v_wallet_id IS NULL THEN
    INSERT INTO public.cc_wallets(user_id) VALUES(p_user_id) ON CONFLICT (user_id) DO NOTHING RETURNING id INTO v_wallet_id;
    IF v_wallet_id IS NULL THEN SELECT id INTO v_wallet_id FROM public.cc_wallets WHERE user_id=p_user_id FOR UPDATE; END IF;
  END IF;
  IF p_purpose IN ('wallet_funding','advertiser_funding') THEN
    SELECT public.process_wallet_transaction(p_user_id,v_wallet_id,'credit',p_amount,'Wallet funding via Paystack','deposit',p_reference_id,coalesce(v_tx.metadata,'{}'::jsonb),'balance') INTO v_result;
    IF NOT coalesce((v_result->>'success')::boolean,false) THEN RAISE EXCEPTION 'Unable to credit funded wallet'; END IF;
  ELSIF p_purpose IN ('product_purchase','escrow') THEN
    ...
    IF upper(coalesce(v_order.status,''))='COMPLETED' THEN
      UPDATE public.paystack_transactions SET processed_at=now(),updated_at=now() WHERE reference=p_reference;
      INSERT INTO public.analytics_events(event_type,entity_type,entity_id,seller_id,viewer_id,metadata)
      VALUES('duplicate_order_payment','order',v_order.id,v_order.seller_id,v_order.buyer_id,jsonb_build_object('payment_reference',p_reference,'amount',p_amount,'purpose',p_purpose,'requires_reconciliation',true));
    ...
    END IF;
    IF upper(coalesce(v_order.status,''))<>'PENDING' THEN RAISE EXCEPTION 'Marketplace order is not payable in its current state'; END IF;
    IF lower(coalesce(v_order.source_type,''))='affiliate' AND v_order.referrer_id IS NOT NULL AND v_order.referrer_id<>v_order.buyer_id AND coalesce(v_order.affiliate_commission_amount,0)>0 THEN
      SELECT account_status INTO v_referrer_account_status FROM public.users WHERE id=v_order.referrer_id;
      ...
      IF v_affiliate_eligible AND v_order.referral_link_id IS NOT NULL THEN
        v_affiliate_eligible:=EXISTS(SELECT 1 FROM public.referral_links rl WHERE rl.id=v_order.referral_link_id AND rl.user_id=v_order.referrer_id AND lower(coalesce(rl.source_type,'affiliate'))='affiliate' AND (rl.product_id IS NULL OR rl.product_id=v_order.product_id));
      END IF;
    END IF;
    v_effective_affiliate_commission:=CASE WHEN v_affiliate_eligible THEN greatest(0,coalesce(v_order.affiliate_commission_amount,0)) ELSE 0 END;
    v_effective_sales_task_amount:=greatest(0,coalesce(v_order.sales_team_task_amount,0));
    IF v_effective_sales_task_amount<=0 AND lower(coalesce(v_order.source_type,'')) IN ('sales_team','advertiser','pro_advertiser','super_advertiser','partnership') AND coalesce(v_order.admin_task_amount,0)<=0 THEN
      v_effective_sales_task_amount:=greatest(0,coalesce(v_order.final_price,0)-coalesce(v_order.base_price,0)-coalesce(v_order.tier_price,0)-coalesce(v_order.customization_price,0));
    END IF;
    v_seller_earnings:=greatest(0,coalesce(v_order.final_price,0)-coalesce(v_order.admin_task_amount,0)-v_effective_sales_task_amount-v_effective_affiliate_commission);
    SELECT public.process_wallet_transaction(p_user_id,v_wallet_id,'credit',p_amount,'Payment for marketplace order','deposit',p_reference_id,coalesce(v_tx.metadata,'{}'::jsonb),'escrow_balance') INTO v_result;
    ...
    UPDATE public.products SET total_sales=coalesce(total_sales,0)+1,stock_quantity=CASE WHEN stock_quantity IS NULL THEN NULL WHEN stock_quantity>0 THEN stock_quantity-1 ELSE 0 END,updated_at=now() WHERE id=v_order.product_id;
    ...
    IF v_seller_wallet_id IS NULL THEN
      INSERT INTO public.cc_wallets(user_id) VALUES(v_order.seller_id) ON CONFLICT (user_id) DO NOTHING RETURNING id INTO v_seller_wallet_id;
      IF v_seller_wallet_id IS NULL THEN SELECT id INTO v_seller_wallet_id FROM public.cc_wallets WHERE user_id=v_order.seller_id FOR UPDATE; END IF;
    END IF;
    ...
    IF v_effective_affiliate_commission>0 THEN
      ...
      IF v_affiliate_wallet_id IS NULL THEN
        INSERT INTO public.cc_wallets(user_id) VALUES(v_order.referrer_id) ON CONFLICT (user_id) DO NOTHING RETURNING id INTO v_affiliate_wallet_id;
        IF v_affiliate_wallet_id IS NULL THEN SELECT id INTO v_affiliate_wallet_id FROM public.cc_wallets WHERE user_id=v_order.referrer_id FOR UPDATE; END IF;
      END IF;
      ...
      IF v_sale_id IS NOT NULL AND v_order.referral_link_id IS NOT NULL THEN UPDATE public.referral_links SET total_conversions=coalesce(total_conversions,0)+1 WHERE id=v_order.referral_link_id; END IF;
    END IF;
    IF lower(coalesce(v_order.source_type,'')) IN ('sales_team','advertiser','pro_advertiser','super_advertiser','partnership') THEN
      ...
      IF NOT coalesce((v_attributed_split->>'success')::boolean,false) THEN RAISE EXCEPTION 'Unable to distribute attributed commission'; END IF;
    END IF;
    INSERT INTO public.analytics_events(event_type,entity_type,entity_id,seller_id,viewer_id,metadata)
    VALUES('purchase','order',v_order.id,v_order.seller_id,v_order.buyer_id,jsonb_build_object('payment_reference',p_reference,'amount',v_order.final_price,'product_id',v_order.product_id,'source_type',v_order.source_type,'source_level',v_order.source_level,'referrer_id',v_order.referrer_id,'referral_link_id',v_order.referral_link_id,'tracking_code',v_order.tracking_code,'campaign_id',v_order.campaign_id,'sales_team_id',v_order.sales_team_id,'team_member_id',v_order.team_member_id,'team_lead_id',v_order.team_lead_id,'commission',v_commission,'affiliate_eligible_at_payment',v_affiliate_eligible,'effective_sales_task_amount',v_effective_sales_task_amount,'attributed_split',v_attributed_split));
  ELSIF p_purpose IN ('subscription','affiliate_subscription','vendor_subscription') THEN
    SELECT public.process_wallet_transaction(p_user_id,v_wallet_id,'credit',p_amount,'Subscription payment','deposit',p_reference_id,coalesce(v_tx.metadata,'{}'::jsonb),'balance') INTO v_result;
    IF NOT coalesce((v_result->>'success')::boolean,false) THEN RAISE EXCEPTION 'Unable to credit subscription payment'; END IF;
  ELSE
    SELECT public.process_wallet_transaction(p_user_id,v_wallet_id,'credit',p_amount,'Payment received','deposit',p_reference_id,coalesce(v_tx.metadata,'{}'::jsonb),'balance') INTO v_result;
  ...
  RETURN jsonb_build_object('success',true,'order_id',CASE WHEN p_purpose IN ('product_purchase','escrow') THEN p_reference_id ELSE NULL END,'commission',v_commission,'seller_earnings',v_seller_earnings,'attributed_split',v_attributed_split,'wallet_result',v_result,'processed_at',now());
END;
$function$;
...
DECLARE
  ...
  v_seller_original numeric:=0; v_seller_reversed numeric:=0; v_seller_remaining numeric:=0; v_seller_reversal numeric:=0;
  v_affiliate_original numeric:=0; v_affiliate_reversed numeric:=0; v_affiliate_remaining numeric:=0; v_affiliate_reversal numeric:=0;
  v_attributed_reversal numeric:=0; v_seller_wallet uuid; v_affiliate_wallet uuid; v_recipient_wallet uuid; v_result jsonb;
  v_full boolean:=false; v_split_reversal numeric;
BEGIN
  IF coalesce(auth.role(),'')<>'service_role' THEN RAISE EXCEPTION 'Refund processing requires service_role'; END IF;
  ...
  SELECT coalesce(sum(le.amount),0) INTO v_seller_original FROM public.ledger_entries le WHERE le.user_id=o.seller_id AND le.entry_type='credit' AND le.account='seller_earnings' AND le.reference_type='order' AND le.reference_id=o.id;
  SELECT coalesce(sum(le.amount),0) INTO v_seller_reversed FROM public.ledger_entries le WHERE le.user_id=o.seller_id AND le.entry_type='debit' AND le.account='seller_earnings' AND le.reference_type='refund' AND le.metadata->>'order_id'=o.id::text;
  v_seller_remaining:=greatest(0,v_seller_original-v_seller_reversed); v_seller_reversal:=CASE WHEN v_full THEN v_seller_remaining ELSE least(v_seller_remaining,round(v_seller_original*v_ratio,2)) END;
  IF o.referrer_id IS NOT NULL AND o.referrer_id<>o.buyer_id AND lower(coalesce(o.source_type,''))='affiliate' THEN
    SELECT coalesce(sum(le.amount),0) INTO v_affiliate_original FROM public.ledger_entries le WHERE le.user_id=o.referrer_id AND le.entry_type='credit' AND le.account='affiliate_balance' AND le.reference_type='order' AND le.reference_id=o.id;
    SELECT coalesce(sum(le.amount),0) INTO v_affiliate_reversed FROM public.ledger_entries le WHERE le.user_id=o.referrer_id AND le.entry_type='debit' AND le.account='affiliate_balance' AND le.reference_type='refund' AND le.metadata->>'order_id'=o.id::text;
  END IF;
  v_affiliate_remaining:=greatest(0,v_affiliate_original-v_affiliate_reversed); v_affiliate_reversal:=CASE WHEN v_full THEN v_affiliate_remaining ELSE least(v_affiliate_remaining,round(v_affiliate_original*v_ratio,2)) END;
  IF v_seller_reversal>0 THEN
    ...
    SELECT public.process_wallet_transaction(o.seller_id,v_seller_wallet,'debit',v_seller_reversal,'Marketplace refund reversal','refund',r.id,jsonb_build_object('order_id',o.id,'refund_id',r.id,'gateway_reference',r.gateway_reference,'amount',r.amount),'seller_earnings') INTO v_result;
  ...
  IF v_affiliate_reversal>0 THEN
    ...
    SELECT public.process_wallet_transaction(o.referrer_id,v_affiliate_wallet,'debit',v_affiliate_reversal,'Affiliate commission refund reversal','refund',r.id,jsonb_build_object('order_id',o.id,'refund_id',r.id,'gateway_reference',r.gateway_reference,'amount',r.amount,'tracking_code',o.tracking_code,'referral_link_id',o.referral_link_id),'affiliate_balance') INTO v_result;
    IF NOT coalesce((v_result->>'success')::boolean,false) THEN RAISE EXCEPTION 'Unable to reverse affiliate commission'; END IF;
    UPDATE public.users SET balance=greatest(0,coalesce(balance,0)-v_affiliate_reversal),available_balance=greatest(0,coalesce(available_balance,0)-v_affiliate_reversal),affiliate_earnings=greatest(0,coalesce(affiliate_earnings,0)-v_affiliate_reversal) WHERE id=o.referrer_id;
  END IF;
  FOR s IN SELECT * FROM public.commission_splits WHERE order_id=o.id AND status='distributed' FOR UPDATE LOOP
    v_split_reversal:=CASE WHEN v_full THEN greatest(0,coalesce(s.amount,0)-coalesce(s.reversed_amount,0)) ELSE greatest(0,least(coalesce(s.amount,0)-coalesce(s.reversed_amount,0),round(coalesce(s.amount,0)*v_ratio,2))) END;
    IF v_split_reversal>0 THEN
      ...
      UPDATE public.commission_splits SET reversed_amount=least(amount,coalesce(reversed_amount,0)+v_split_reversal) WHERE id=s.id;
      v_attributed_reversal:=v_attributed_reversal+v_split_reversal;
    END IF;
  END LOOP;
  UPDATE public.sales_records SET status=CASE WHEN v_full THEN 'refunded' ELSE status END,commission_amount=greatest(0,coalesce(commission_amount,0)-v_affiliate_reversal) WHERE order_id=o.id AND status NOT IN ('cancelled');
  IF v_full THEN
  ...
  END IF;
  UPDATE public.refund_records SET financial_processed_at=now(),financial_reversal_metadata=jsonb_build_object('order_id',o.id,'refund_amount',r.amount,'seller_original_credit',v_seller_original,'seller_reversal',v_seller_reversal,'affiliate_original_credit',v_affiliate_original,'affiliate_reversal',v_affiliate_reversal,'attributed_commission_reversal',v_attributed_reversal,'cumulative_refunded_before',v_total_refunded,'processed_at',now()),updated_at=now() WHERE id=r.id;
  INSERT INTO public.analytics_events(event_type,entity_type,entity_id,seller_id,viewer_id,metadata)
  VALUES('refund_processed','refund',r.id,o.seller_id,o.buyer_id,jsonb_build_object('order_id',o.id,'refund_amount',r.amount,'seller_reversal',v_seller_reversal,'affiliate_reversal',v_affiliate_reversal,'attributed_commission_reversal',v_attributed_reversal,'source_type',o.source_type,'referrer_id',o.referrer_id,'referral_link_id',o.referral_link_id,'campaign_id',o.campaign_id,'sales_team_id',o.sales_team_id,'team_member_id',o.team_member_id,'team_lead_id',o.team_lead_id));
  RETURN jsonb_build_object('success',true,'refund_id',r.id,'order_id',o.id,'seller_reversal',v_seller_reversal,'affiliate_reversal',v_affiliate_reversal,'attributed_commission_reversal',v_attributed_reversal,'fully_refunded',v_full);
END;
$function$;
