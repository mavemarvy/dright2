...
-- Replace permissive policies that allowed any authenticated user to impersonate staff
-- merely by setting staff_id = auth.uid().
...
begin
  ...
  insert into public.customer_timelines(
    ...
    new.user_id,
    'customer_care_in_app_message',
    'support',
    'DRIGHT Support sent an in-app message',
...
-- Realtime keeps the admin interaction history current without manual page refreshes.
do $$
begin
  if not exists (
