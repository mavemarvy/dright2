-- Seller products performance list
CREATE OR REPLACE FUNCTION get_seller_products_performance(
  p_seller_id UUID,
...
BEGIN
  ...
  LEFT JOIN (SELECT product_id, ROUND(avg(rating), 2) AS avg_rating, count(*) AS cnt FROM product_reviews GROUP BY product_id) r ON r.product_id = p.id
...
GRANT EXECUTE ON FUNCTION get_seller_products_performance TO authenticated;
