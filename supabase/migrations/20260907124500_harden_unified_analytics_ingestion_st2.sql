-- ST-2 hardening: only the existing track-event Edge Function may invoke the analytics RPC.
-- Anonymous/signed-in browsers still use track-event; the Edge Function validates any user JWT
-- and passes only the verified viewer id through an internal field stripped before storage.
...
declare
  v_viewer_id uuid;
  v_metadata jsonb := coalesce(p_metadata,'{}'::jsonb) - '_verified_viewer_id';
...
begin
  if coalesce(auth.jwt()->>'role','') <> 'service_role' then
    raise exception 'analytics ingestion requires server authority' using errcode='42501';
  end if;

  begin
    v_viewer_id := nullif(p_metadata->>'_verified_viewer_id','')::uuid;
  exception when invalid_text_representation then
    raise exception 'invalid verified viewer id' using errcode='22023';
  end;

  v_page_path := nullif(left(coalesce(v_metadata->>'page_path',''),500),'');
  ...
  if v_cooldown is not null and p_entity_id is not null then
    ...
    where ae.entity_id=p_entity_id and ae.entity_type=p_entity_type and ae.event_type=p_event_type
  ...
  insert into public.analytics_events(
    event_type,entity_type,entity_id,seller_id,viewer_id,session_id,ip_hash,device_hash,browser,country,city,
    referrer,source,metadata,is_bot,device_type,os,browser_name,state,language,timezone,session_duration,is_bounce,keywords
  ) values(
    left(btrim(p_event_type),80),left(coalesce(nullif(btrim(p_entity_type),''),'platform'),40),p_entity_id,p_seller_id,
    v_viewer_id,p_session_id,encode(extensions.digest(coalesce(current_setting('request.headers',true),''),'sha256'),'hex'),
    ...
    nullif(left(coalesce(p_referrer,''),1000),''),left(coalesce(nullif(btrim(p_source),''),'direct'),80),v_metadata,false,
    nullif(left(coalesce(p_device_type,''),30),''),nullif(left(coalesce(p_os,''),60),''),
    nullif(left(coalesce(p_browser_name,''),60),''),nullif(left(coalesce(p_state,''),100),''),
    nullif(left(coalesce(p_language,''),30),''),nullif(left(coalesce(p_timezone,''),100),''),
    p_session_duration,coalesce(p_is_bounce,false),nullif(left(coalesce(p_keywords,''),500),'')
  ...
  if p_session_id is not null and btrim(p_session_id)<>'' then
    insert into public.analytics_sessions(
      session_id,user_id,device_type,os,browser,language,timezone,country,entry_page,exit_page,page_views,events_count,
  ...
  if v_legacy_event_type is not null and p_entity_id is not null and p_entity_type in ('product','service','job','course','digital_download') then
    if not exists(
      ...
      values(p_entity_id,p_entity_type,v_viewer_id,v_legacy_event_type,v_metadata,p_session_id,v_created_at,p_source);
    end if;
  end if;

  if p_event_type='search' and nullif(btrim(coalesce(v_metadata->>'query','')),'') is not null then
    insert into public.search_history(user_id,query,category,filters,result_count,clicked_listing_id)
    values(
      v_viewer_id,left(v_metadata->>'query',500),nullif(left(coalesce(v_metadata->>'category',''),200),''),
      case when jsonb_typeof(v_metadata->'filters')='object' then v_metadata->'filters' else null end,
      case when coalesce(v_metadata->>'result_count','')~'^\d+$' then (v_metadata->>'result_count')::integer else null end,
      case when coalesce(v_metadata->>'clicked_listing_id','')~*'^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then (v_metadata->>'clicked_listing_id')::uuid else null end
    );
  end if;
...
revoke all on function public.track_analytics_event(text,text,uuid,uuid,text,text,text,text,text,text,text,jsonb,boolean,text,text,text,text,text,text,integer,boolean,text) from public;
revoke all on function public.track_analytics_event(text,text,uuid,uuid,text,text,text,text,text,text,text,jsonb,boolean,text,text,text,text,text,text,integer,boolean,text) from anon;
revoke all on function public.track_analytics_event(text,text,uuid,uuid,text,text,text,text,text,text,text,jsonb,boolean,text,text,text,text,text,text,integer,boolean,text) from authenticated;
grant execute on function public.track_analytics_event(text,text,uuid,uuid,text,text,text,text,text,text,text,jsonb,boolean,text,text,text,text,text,text,integer,boolean,text) to service_role;
