...
BEGIN
  ...
  IF v_wallet_id IS NULL THEN
    INSERT INTO public.cc_wallets (user_id) VALUES (p_user_id) RETURNING id INTO v_wallet_id;
  END IF;

  IF p_purpose = 'wallet_funding' OR p_purpose = 'advertiser_funding' THEN
    SELECT public.process_wallet_transaction(p_user_id, v_wallet_id, 'credit', p_amount,
      'Wallet funding via Paystack', 'deposit', p_reference_id, p_metadata, 'balance') INTO v_result;
  ELSIF p_purpose = 'product_purchase' OR p_purpose = 'escrow' THEN
    SELECT public.process_wallet_transaction(p_user_id, v_wallet_id, 'credit', p_amount,
      'Payment for marketplace order', 'deposit', p_reference_id, p_metadata, 'escrow_balance') INTO v_result;
    ...
    -- Successful payment is the sole paid-order conversion boundary.
    IF v_order.status <> 'COMPLETED' THEN
      UPDATE public.orders
      SET status = 'COMPLETED', completed_at = COALESCE(completed_at, now())
      WHERE id = v_order.id;

      -- Increment product sales and decrement finite inventory exactly once.
      ...
      -- Seller earnings use the authoritative order amounts; never trust browser values.
      v_seller_earnings := GREATEST(0,
        COALESCE(v_order.final_price, 0)
        - COALESCE(v_order.admin_task_amount, 0)
        - COALESCE(v_order.sales_team_task_amount, 0)
      ...
      IF v_seller_wallet_id IS NULL THEN
        INSERT INTO public.cc_wallets (user_id) VALUES (v_order.seller_id) RETURNING id INTO v_seller_wallet_id;
      END IF;

      IF v_seller_earnings > 0 THEN
        SELECT public.process_wallet_transaction(v_order.seller_id, v_seller_wallet_id, 'credit', v_seller_earnings,
          'Marketplace sale earnings', 'order', v_order.id,
          jsonb_build_object('payment_reference', p_reference, 'product_id', v_order.product_id), 'seller_earnings') INTO v_result;
        IF NOT COALESCE((v_result->>'success')::boolean, false) THEN
          RAISE EXCEPTION 'Unable to credit seller earnings';
        END IF;
      END IF;

      -- Keep the existing aggregate user fields synchronized with the ledger.
      UPDATE public.users
      ...
      -- Affiliate conversion is created once, only when the order has affiliate attribution.
      IF v_order.source_type = 'affiliate' AND v_order.referrer_id IS NOT NULL
        ...
        IF v_affiliate_wallet_id IS NULL THEN
          INSERT INTO public.cc_wallets (user_id) VALUES (v_order.referrer_id) RETURNING id INTO v_affiliate_wallet_id;
        END IF;

        SELECT public.process_wallet_transaction(v_order.referrer_id, v_affiliate_wallet_id, 'credit', v_commission,
          'Affiliate commission', 'order', v_order.id,
          jsonb_build_object('payment_reference', p_reference, 'product_id', v_order.product_id,
            'tracking_code', v_order.tracking_code, 'referral_link_id', v_order.referral_link_id), 'affiliate_balance') INTO v_result;
        IF NOT COALESCE((v_result->>'success')::boolean, false) THEN
        ...
        SELECT v_order.id, v_order.referrer_id,
               COALESCE(u.full_name, 'Buyer'), p.name, v_commission, v_order.final_price,
               v_order.referrer_id, v_order.referrer_role, v_order.product_id, 'paid', CURRENT_DATE
        FROM public.users u
      ...
      INSERT INTO public.analytics_events (
        ...
        'purchase', 'order', v_order.id, v_order.seller_id, v_order.buyer_id,
        jsonb_build_object(
  ...
  ELSIF p_purpose = 'subscription' OR p_purpose = 'affiliate_subscription' OR p_purpose = 'vendor_subscription' THEN
    SELECT public.process_wallet_transaction(p_user_id, v_wallet_id, 'credit', p_amount,
      'Subscription payment', 'deposit', p_reference_id, p_metadata, 'balance') INTO v_result;
  ELSE
    SELECT public.process_wallet_transaction(p_user_id, v_wallet_id, 'credit', p_amount,
      'Payment received', 'deposit', p_reference_id, p_metadata, 'balance') INTO v_result;
  END IF;
