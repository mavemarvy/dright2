create table if not exists public.sales_progression_weekly (
  ...
  marketer_level_before integer not null default 0,
  marketer_level_after integer not null default 0,
  advertiser_grade_before text,
  advertiser_grade_after text,
  ...
  failure_streak_before integer not null default 0,
  failure_streak_after integer not null default 0,
...
alter table public.users add column if not exists consecutive_week_failures integer not null default 0;

create index if not exists idx_sales_progression_weekly_user_period on public.sales_progression_weekly(user_id, period_start desc);
create index if not exists idx_sales_progression_weekly_stage on public.sales_progression_weekly(stage_after, evaluated_at desc);

alter table public.sales_progression_weekly enable row level security;
revoke all on table public.sales_progression_weekly from anon, authenticated;
grant all on table public.sales_progression_weekly to service_role;
...
begin
  ...
  loop
    ...
    v_target := 0;
    v_target_met := false;
    v_level := coalesce(u.marketer_level, 0);
    ...
    if u.advertiser_status = 'approved' and u.advertiser_grade is not null then
      v_grade := u.advertiser_grade;
      if v_grade = 'A' then v_target := 500; v_req_total := 2000; v_downgrade_to := 'marketer';
      elsif v_grade = 'B' then v_target := 500; v_req_total := 5000; v_downgrade_to := 'A';
      elsif v_grade = 'C' then v_target := 600; v_req_total := 10000; v_downgrade_to := 'B';
      elsif v_grade = 'Pro' then v_target := 1000; v_req_total := 50000; v_downgrade_to := 'B';
      elsif v_grade = 'Super' then v_target := 1000; v_req_total := 50000; v_downgrade_to := 'Pro';
      elsif v_grade = 'Partnership' then v_target := 4000; v_req_total := 100000; v_downgrade_to := 'Super';
      else v_target := 0; v_req_total := 0; v_downgrade_to := null;
      end if;
      ...
      if v_target_met then
        ...
        v_fail_after := 0;
        if v_grade = 'A' then v_next_grade := 'B'; v_req_weekly := 500; v_req_total := 5000;
        elsif v_grade = 'B' then v_next_grade := 'C'; v_req_weekly := 600; v_req_total := 10000;
        elsif v_grade = 'C' then v_next_grade := 'Pro'; v_req_weekly := 1000; v_req_total := 50000;
        elsif v_grade = 'Pro' then v_next_grade := 'Super'; v_req_weekly := 1000; v_req_total := 50000;
        elsif v_grade = 'Super' then v_next_grade := 'Partnership'; v_req_weekly := 4000; v_req_total := 100000;
        else v_next_grade := null; v_req_weekly := null; v_req_total := null;
        end if;
        if v_next_grade is not null and v_streak_after >= 4 and v_total_sales >= v_req_total then
          v_action := 'upgrade';
          v_next_grade := v_next_grade;
          v_streak_after := 0;
          v_fail_after := 0;
      ...
      v_level := greatest(0, least(5, v_level));
    ...
    if v_action = 'upgrade' and u.advertiser_status = 'approved' and u.advertiser_grade is not null then
      update public.users set advertiser_grade=v_next_grade, consecutive_weeks_streak=v_streak_after,
        consecutive_week_failures=v_fail_after, weekly_sales_count=0, last_weekly_reset_at=v_now,
        downgraded_at=null where id=u.id;
    elsif v_action = 'downgrade' and v_downgrade_to = 'marketer' then
      update public.users set advertiser_grade=null, advertiser_status='none', marketer_status='approved',
        marketer_level=5, consecutive_weeks_streak=0, consecutive_week_failures=0,
        weekly_sales_count=0, last_weekly_reset_at=v_now, downgraded_at=v_now where id=u.id;
    elsif v_action = 'downgrade' then
      update public.users set advertiser_grade=v_next_grade, consecutive_weeks_streak=0,
        consecutive_week_failures=0, weekly_sales_count=0, last_weekly_reset_at=v_now,
        downgraded_at=v_now where id=u.id;
    else
      update public.users set marketer_level=v_next_level, consecutive_weeks_streak=v_streak_after,
        consecutive_week_failures=v_fail_after, weekly_sales_count=0, last_weekly_reset_at=v_now
        where id=u.id;
    end if;
...
select cron.schedule('dright2-weekly-sales-progression','0 23 * * 0','select public.run_weekly_sales_progression();');