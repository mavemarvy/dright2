/*
  ...
  Forward-only, additive migration. Existing auth/users remain authoritative.
*/

-- ---------------------------------------------------------------------
-- Shared permission helper: existing role permissions + per-admin override.
-- Explicit per-admin deny wins over a role grant. Super Admin remains final authority.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF v_user.admin_role = 'super_admin' THEN
    RETURN true;
  END IF;

  SELECT ap.is_granted, true
    INTO v_override, v_has_override
  ...
  IF v_role_id IS NULL AND v_user.admin_role IS NOT NULL THEN
    ...
    WHERE slug = v_user.admin_role AND is_deleted = false AND is_archived = false
    LIMIT 1;
  END IF;
...
SET label = EXCLUDED.label, description = EXCLUDED.description, is_active = true, is_deleted = false;

-- Super Admin always receives every permission. Safe default review roles receive narrowly scoped access.
...
JOIN public.permissions p ON (
  (r.slug = 'user_management_admin' AND (p.module,p.action) IN (
    ('users','view'),('users','view_sensitive'),('questionnaires','view'),('questionnaires','review'),('eligibility','view')
  ))
  OR (r.slug IN ('trust_safety_admin','security_admin','security_manager') AND (p.module,p.action) IN (
    ('kyc','view'),('kyc','review'),('kyc','verify'),('kyc','reject'),('kyc','request_resubmission'),
    ('kyc','view_documents'),('kyc','audit_view'),('users','view_sensitive'),('questionnaires','view'),('eligibility','view'),
    ('badges','view_evidence'),('badges','review_evidence')
  ))
  OR (r.slug = 'badge_trust_admin' AND (p.module,p.action) IN (
    ('badges','view'),('badges','assign'),('badges','manage'),('badges','view_evidence'),('badges','review_evidence'),('badges','manage_ranking'),('eligibility','view')
  ))
)
...
-- ---------------------------------------------------------------------
-- Username: case-insensitive authority, normalization, reserved names.
-- Existing duplicate check was clean before this migration was authored.
-- ---------------------------------------------------------------------
...
INSERT INTO public.reserved_usernames (username, reason) VALUES
  ('admin','system reserved'),('administrator','system reserved'),('support','system reserved'),
  ('security','system reserved'),('dright','brand reserved'),('drightadmin','brand reserved'),
  ('help','system reserved'),('api','system reserved'),('root','system reserved'),
  ('moderator','system reserved'),('system','system reserved'),('staff','system reserved')
...
USING (public.has_dright_permission('users','manage_roles'))
WITH CHECK (public.has_dright_permission('users','manage_roles'));

CREATE OR REPLACE FUNCTION public.normalize_dright_username(p_username text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = public, pg_temp
AS $$
  SELECT trim(both '_' from regexp_replace(lower(trim(coalesce(p_username,''))), '[^a-z0-9_]+', '_', 'g'));
$$;
...
AS $$
DECLARE
  v_name text := public.normalize_dright_username(p_username);
  v_available boolean;
BEGIN
  IF length(v_name) < 3 OR length(v_name) > 30 OR v_name !~ '^[a-z0-9_]+$' THEN
    RETURN jsonb_build_object('normalized',v_name,'available',false,'reason','invalid_format');
  END IF;
  IF EXISTS (SELECT 1 FROM public.reserved_usernames WHERE username = v_name AND is_active = true) THEN
    RETURN jsonb_build_object('normalized',v_name,'available',false,'reason','reserved');
  END IF;
...
BEGIN
  ...
  IF length(v_name) < 3 OR length(v_name) > 30 OR v_name !~ '^[a-z0-9_]+$' THEN
    RAISE EXCEPTION 'username must be 3-30 characters using letters, numbers or underscore';
  END IF;
  IF EXISTS (SELECT 1 FROM public.reserved_usernames WHERE username=v_name AND is_active=true) THEN
    RAISE EXCEPTION 'username is reserved';
  END IF;
  UPDATE public.users SET username=v_name, updated_at=now() WHERE id=v_uid;
...
-- ---------------------------------------------------------------------
-- Private onboarding identity/profile fields. Never place DOB in public user rows.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.user_private_profiles (
  ...
  date_of_birth date,
  intended_profiles text[] NOT NULL DEFAULT ARRAY['buyer']::text[],
  interests text[] NOT NULL DEFAULT '{}'::text[],
  onboarding_status text NOT NULL DEFAULT 'in_progress'
    CHECK (onboarding_status IN ('in_progress','submitted','completed','changes_required')),
  onboarding_completed_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT user_private_country_iso2 CHECK (country_iso2 IS NULL OR country_iso2 ~ '^[A-Z]{2}$'),
  CONSTRAINT user_private_dob_not_future CHECK (date_of_birth IS NULL OR date_of_birth <= current_date)
);
CREATE INDEX IF NOT EXISTS idx_user_private_profiles_country ON public.user_private_profiles(country_iso2);
ALTER TABLE public.user_private_profiles ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS user_private_profiles_own_read ON public.user_private_profiles;
CREATE POLICY user_private_profiles_own_read ON public.user_private_profiles
FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS user_private_profiles_admin_read ON public.user_private_profiles;
CREATE POLICY user_private_profiles_admin_read ON public.user_private_profiles
FOR SELECT TO authenticated USING (public.has_dright_permission('users','view_sensitive'));
DROP POLICY IF EXISTS user_private_profiles_own_insert ON public.user_private_profiles;
CREATE POLICY user_private_profiles_own_insert ON public.user_private_profiles
FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS user_private_profiles_own_update ON public.user_private_profiles;
CREATE POLICY user_private_profiles_own_update ON public.user_private_profiles
FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

CREATE TABLE IF NOT EXISTS public.age_eligibility_rules (
  profile_type text PRIMARY KEY,
  minimum_age integer NOT NULL CHECK (minimum_age BETWEEN 0 AND 100),
  reason text,
...
DROP POLICY IF EXISTS age_rules_read ON public.age_eligibility_rules;
CREATE POLICY age_rules_read ON public.age_eligibility_rules
FOR SELECT TO anon, authenticated USING (is_active = true);
DROP POLICY IF EXISTS age_rules_manage ON public.age_eligibility_rules;
CREATE POLICY age_rules_manage ON public.age_eligibility_rules
FOR ALL TO authenticated
USING (public.has_dright_permission('kyc','manage_rules'))
WITH CHECK (public.has_dright_permission('kyc','manage_rules'));

INSERT INTO public.age_eligibility_rules (profile_type, minimum_age, reason)
VALUES
 ('buyer',13,'Base account access; stricter activity rules may apply'),
 ('affiliate',18,'Commission and withdrawal activity'),
 ('seller',18,'Commercial selling and contractual activity'),
 ('vendor',18,'Commercial selling and contractual activity'),
 ('service_provider',18,'Contractual service activity'),
 ('employer',18,'Job-posting and hiring activity'),
 ('creator',13,'Base creator access; monetization may require 18+'),
 ('course_creator',18,'Monetized course publishing'),
 ('task_worker',18,'Paid task activity'),
 ('task_creator',18,'Paid task contracting'),
 ('marketer',18,'Commercial marketing activity')
ON CONFLICT (profile_type) DO NOTHING;

CREATE OR REPLACE FUNCTION public.dright_age_on(p_dob date, p_on date DEFAULT current_date)
RETURNS integer LANGUAGE sql IMMUTABLE SET search_path=public,pg_temp AS $$
  SELECT CASE WHEN p_dob IS NULL THEN NULL ELSE extract(year from age(p_on,p_dob))::int END;
$$;

-- ---------------------------------------------------------------------
-- Versioned dynamic questionnaire model.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.questionnaire_definitions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  questionnaire_key text NOT NULL,
  name text NOT NULL,
  applicable_profile_type text NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS questionnaire_one_active_version
  ON public.questionnaire_definitions(questionnaire_key) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_questionnaire_definitions_profile ON public.questionnaire_definitions(applicable_profile_type,is_active);
...
CREATE TABLE IF NOT EXISTS public.questionnaire_questions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  questionnaire_id uuid NOT NULL REFERENCES public.questionnaire_definitions(id) ON DELETE RESTRICT,
  question_key text NOT NULL,
  label text NOT NULL,
  description text,
  answer_type text NOT NULL CHECK (answer_type IN ('short_text','long_text','yes_no','single_select','multi_select','number','date','url','email','document_reference')),
...
CREATE INDEX IF NOT EXISTS idx_questionnaire_questions_def_order ON public.questionnaire_questions(questionnaire_id,sort_order);
...
CREATE TABLE IF NOT EXISTS public.questionnaire_submissions (
  ...
  status text NOT NULL DEFAULT 'submitted' CHECK (status IN ('draft','submitted','under_review','approved','more_information_required','returned_for_changes','rejected','reopened')),
...
CREATE INDEX IF NOT EXISTS idx_questionnaire_submissions_user ON public.questionnaire_submissions(user_id,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_questionnaire_submissions_status ON public.questionnaire_submissions(status,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_questionnaire_submissions_profile ON public.questionnaire_submissions(profile_type,status);
...
CREATE TABLE IF NOT EXISTS public.questionnaire_answers (
  ...
  question_id uuid REFERENCES public.questionnaire_questions(id) ON DELETE SET NULL,
  question_key text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.questionnaire_review_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  submission_id uuid NOT NULL REFERENCES public.questionnaire_submissions(id) ON DELETE CASCADE,
  reviewer_id uuid NOT NULL REFERENCES public.users(id),
  action text NOT NULL CHECK (action IN ('under_review','approved','more_information_required','returned_for_changes','rejected','reopened')),
...
CREATE INDEX IF NOT EXISTS idx_questionnaire_review_events_submission ON public.questionnaire_review_events(submission_id,created_at DESC);
ALTER TABLE public.questionnaire_review_events ENABLE ROW LEVEL SECURITY;

-- Public can render only active questionnaire definitions/questions during signup.
DROP POLICY IF EXISTS questionnaire_definitions_public_read ON public.questionnaire_definitions;
CREATE POLICY questionnaire_definitions_public_read ON public.questionnaire_definitions
FOR SELECT TO anon, authenticated USING (is_active = true);
DROP POLICY IF EXISTS questionnaire_definitions_manage ON public.questionnaire_definitions;
CREATE POLICY questionnaire_definitions_manage ON public.questionnaire_definitions
FOR ALL TO authenticated USING (public.has_dright_permission('questionnaires','manage'))
WITH CHECK (public.has_dright_permission('questionnaires','manage'));

DROP POLICY IF EXISTS questionnaire_questions_public_read ON public.questionnaire_questions;
CREATE POLICY questionnaire_questions_public_read ON public.questionnaire_questions
FOR SELECT TO anon, authenticated USING (
  is_active = true AND EXISTS (SELECT 1 FROM public.questionnaire_definitions qd WHERE qd.id=questionnaire_id AND qd.is_active=true)
);
DROP POLICY IF EXISTS questionnaire_questions_manage ON public.questionnaire_questions;
CREATE POLICY questionnaire_questions_manage ON public.questionnaire_questions
FOR ALL TO authenticated USING (public.has_dright_permission('questionnaires','manage'))
WITH CHECK (public.has_dright_permission('questionnaires','manage'));

DROP POLICY IF EXISTS questionnaire_submissions_own_read ON public.questionnaire_submissions;
CREATE POLICY questionnaire_submissions_own_read ON public.questionnaire_submissions
FOR SELECT TO authenticated USING (user_id=auth.uid());
DROP POLICY IF EXISTS questionnaire_submissions_admin_read ON public.questionnaire_submissions;
CREATE POLICY questionnaire_submissions_admin_read ON public.questionnaire_submissions
FOR SELECT TO authenticated USING (public.has_dright_permission('questionnaires','view'));
DROP POLICY IF EXISTS questionnaire_submissions_admin_update ON public.questionnaire_submissions;
CREATE POLICY questionnaire_submissions_admin_update ON public.questionnaire_submissions
FOR UPDATE TO authenticated USING (public.has_dright_permission('questionnaires','review'))
WITH CHECK (public.has_dright_permission('questionnaires','review'));

DROP POLICY IF EXISTS questionnaire_answers_own_read ON public.questionnaire_answers;
CREATE POLICY questionnaire_answers_own_read ON public.questionnaire_answers
FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.questionnaire_submissions s WHERE s.id=submission_id AND s.user_id=auth.uid()));
DROP POLICY IF EXISTS questionnaire_answers_admin_read ON public.questionnaire_answers;
CREATE POLICY questionnaire_answers_admin_read ON public.questionnaire_answers
FOR SELECT TO authenticated USING (public.has_dright_permission('questionnaires','view'));

DROP POLICY IF EXISTS questionnaire_review_events_own_read ON public.questionnaire_review_events;
CREATE POLICY questionnaire_review_events_own_read ON public.questionnaire_review_events
FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.questionnaire_submissions s WHERE s.id=submission_id AND s.user_id=auth.uid()));
DROP POLICY IF EXISTS questionnaire_review_events_admin_read ON public.questionnaire_review_events;
CREATE POLICY questionnaire_review_events_admin_read ON public.questionnaire_review_events
FOR SELECT TO authenticated USING (public.has_dright_permission('questionnaires','view'));

-- Seed profile questionnaires. Versioning ensures later edits never rewrite historical answers.
INSERT INTO public.questionnaire_definitions (questionnaire_key,name,applicable_profile_type,version,description)
VALUES
 ('buyer_onboarding','Buyer onboarding','buyer',1,'Short buyer discovery preferences'),
 ('affiliate_application','Affiliate application','affiliate',1,'Affiliate experience, audience and promotion plan'),
 ('employer_application','Employer application','employer',1,'Employer and organization information'),
 ('service_provider_application','Freelancer / Service Provider application','service_provider',1,'Skills, availability and professional experience'),
 ('seller_application','Seller / Vendor application','seller',1,'Seller and store information'),
 ('task_worker_application','Task Worker / Earner application','task_worker',1,'Skills, task categories and availability'),
 ('task_creator_application','Task Creator application','task_creator',1,'Task creation needs and expected activity'),
 ('marketer_application','Marketer application','marketer',1,'Marketing and sales background'),
 ('creator_application','Creator application','creator',1,'Creator content, expertise and goals'),
 ('course_creator_application','Course Creator application','course_creator',1,'Course subject, expertise and monetization goals')
ON CONFLICT (questionnaire_key,version) DO NOTHING;

-- Helper for concise seed inserts.
CREATE OR REPLACE FUNCTION public.seed_question(
  p_key text, p_question_key text, p_label text, p_type text,
  p_required boolean, p_order integer, p_options jsonb DEFAULT '[]'::jsonb,
...
AS $$
BEGIN
  INSERT INTO public.questionnaire_questions(questionnaire_id,question_key,label,description,answer_type,options,is_required,sort_order)
  SELECT id,p_question_key,p_label,p_description,p_type,p_options,p_required,p_order
  FROM public.questionnaire_definitions
  WHERE questionnaire_key=p_key AND version=1
...
-- ---------------------------------------------------------------------
-- Authoritative onboarding submission. The authenticated identity is always auth.uid().
-- p_answers is keyed by questionnaire_key then question_key.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  FOREACH v_profile IN ARRAY p_intended_profiles LOOP
    ...
    IF v_rule.profile_type IS NOT NULL AND public.dright_age_on(p_date_of_birth) < v_rule.minimum_age THEN
  ...
  FOREACH v_profile IN ARRAY p_intended_profiles LOOP
    ...
    FOR v_question IN SELECT * FROM public.questionnaire_questions WHERE questionnaire_id=v_q.id AND is_active=true ORDER BY sort_order LOOP
      ...
      IF v_question.is_required AND (v_answer IS NULL OR v_answer='null'::jsonb OR v_answer='""'::jsonb OR v_answer='[]'::jsonb) THEN
        RAISE EXCEPTION 'required questionnaire answer missing: %', v_question.question_key;
      END IF;
    END LOOP;
    ...
    VALUES(v_uid,v_profile,v_q.id,v_q.version,'submitted',now()) RETURNING id INTO v_submission;

    INSERT INTO public.questionnaire_answers(submission_id,question_id,question_key,answer_value,question_snapshot)
    SELECT v_submission, qq.id, qq.question_key,
      coalesce(v_profile_answers -> qq.question_key,'null'::jsonb),
      jsonb_build_object(
        'question_key',qq.question_key,'label',qq.label,'description',qq.description,'answer_type',qq.answer_type,
        'options',qq.options,'required',qq.is_required,'sort_order',qq.sort_order,
        'conditional_rules',qq.conditional_rules,'validation_rules',qq.validation_rules,
        'questionnaire_key',v_q.questionnaire_key,'questionnaire_version',v_q.version
      )
...
) RETURNS public.questionnaire_submissions
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public,pg_temp
AS $$
DECLARE
  v_old text;
  v_new text;
  v_row public.questionnaire_submissions%ROWTYPE;
BEGIN
  ...
  IF p_action IN ('more_information_required','returned_for_changes','rejected') AND nullif(trim(coalesce(p_user_visible_reason,'')),'') IS NULL THEN
    RAISE EXCEPTION 'user-visible reason is required';
  END IF;

  SELECT status INTO v_old FROM public.questionnaire_submissions WHERE id=p_submission_id FOR UPDATE;
  IF v_old IS NULL THEN RAISE EXCEPTION 'submission not found'; END IF;
  v_new := p_action;

  UPDATE public.questionnaire_submissions
  SET status=v_new,reviewer_id=auth.uid(),reviewed_at=now(),user_visible_reason=p_user_visible_reason,
      internal_notes=p_internal_note,updated_at=now()
  WHERE id=p_submission_id RETURNING * INTO v_row;

  INSERT INTO public.questionnaire_review_events(submission_id,reviewer_id,action,previous_status,new_status,user_visible_reason,internal_note)
  VALUES(p_submission_id,auth.uid(),p_action,v_old,v_new,p_user_visible_reason,p_internal_note);

  INSERT INTO public.admin_activity_logs(admin_id,action,resource_type,resource_id,details)
  SELECT auth.uid(),'questionnaire_'||p_action,'questionnaire_submission',p_submission_id,
         jsonb_build_object('previous_status',v_old,'new_status',v_new)
  WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='admin_activity_logs');

  RETURN v_row;
END;
$$;
REVOKE ALL ON FUNCTION public.review_questionnaire_submission(uuid,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.review_questionnaire_submission(uuid,text,text,text) TO authenticated;
