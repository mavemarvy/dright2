...
BEGIN
  ...
  SELECT allowed_reactions INTO v_allowed FROM public.global_announcements WHERE id = p_content_id AND is_active = true AND (content_kind = 'news' OR show_in_news = true);
  IF NOT FOUND THEN RAISE EXCEPTION 'Post not found'; END IF;
  IF NOT (v_reaction = ANY(v_allowed)) THEN RAISE EXCEPTION 'This reaction is not enabled for the post'; END IF;
  SELECT reaction INTO v_existing FROM public.global_content_reactions WHERE content_id = p_content_id AND user_id = v_user_id;
  IF v_existing = v_reaction THEN
    DELETE FROM public.global_content_reactions WHERE content_id = p_content_id AND user_id = v_user_id;
    v_current := NULL;
  ELSE
    INSERT INTO public.global_content_reactions(content_id, user_id, reaction, created_at, updated_at) VALUES (p_content_id, v_user_id, v_reaction, now(), now())
    ON CONFLICT (content_id, user_id) DO UPDATE SET reaction = EXCLUDED.reaction, updated_at = now();
    v_current := v_reaction;
  END IF;
  SELECT count(*) INTO v_count FROM public.global_content_reactions WHERE content_id = p_content_id;
  SELECT COALESCE(jsonb_object_agg(x.reaction, x.cnt), '{}'::jsonb) INTO v_breakdown FROM (SELECT reaction, count(*)::bigint AS cnt FROM public.global_content_reactions WHERE content_id = p_content_id GROUP BY reaction) x;
...
SET search_path TO 'public'
AS $$
DECLARE v_user_id uuid := auth.uid(); v_saved boolean; v_count bigint;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.global_announcements WHERE id = p_content_id AND is_active = true AND (content_kind = 'news' OR show_in_news = true)) THEN RAISE EXCEPTION 'Post not found'; END IF;
  IF EXISTS (SELECT 1 FROM public.global_content_saves WHERE content_id = p_content_id AND user_id = v_user_id) THEN
    DELETE FROM public.global_content_saves WHERE content_id = p_content_id AND user_id = v_user_id; v_saved := false;
  ELSE
    INSERT INTO public.global_content_saves(content_id, user_id) VALUES (p_content_id, v_user_id) ON CONFLICT DO NOTHING; v_saved := true;
  END IF;
  SELECT count(*) INTO v_count FROM public.global_content_saves WHERE content_id = p_content_id;
...
SET search_path TO 'public'
AS $$
DECLARE v_user_id uuid := auth.uid(); v_comment_id uuid; v_count bigint;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  IF length(trim(COALESCE(p_body, ''))) = 0 OR length(trim(p_body)) > 2000 THEN RAISE EXCEPTION 'Comment must be between 1 and 2000 characters'; END IF;
  ...
  INSERT INTO public.global_content_comments(content_id, user_id, body) VALUES (p_content_id, v_user_id, trim(p_body)) RETURNING id INTO v_comment_id;
  SELECT count(*) INTO v_count FROM public.global_content_comments WHERE content_id = p_content_id AND status = 'visible';
...
SET search_path TO 'public'
AS $$
DECLARE v_user_id uuid := auth.uid(); v_content_id uuid; v_count bigint;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  DELETE FROM public.global_content_comments WHERE id = p_comment_id AND user_id = v_user_id RETURNING content_id INTO v_content_id;
  IF v_content_id IS NULL THEN RETURN jsonb_build_object('deleted', false); END IF;
  SELECT count(*) INTO v_count FROM public.global_content_comments WHERE content_id = v_content_id AND status = 'visible';
...
AS $$
  SELECT cm.id,cm.user_id,cm.body,cm.created_at,COALESCE(NULLIF(u.full_name, ''), split_part(u.email, '@', 1), 'DRIGHT user'),u.avatar_url,(cm.user_id = auth.uid())
  FROM public.global_content_comments cm JOIN public.global_announcements c ON c.id = cm.content_id LEFT JOIN public.users u ON u.id = cm.user_id
  WHERE cm.content_id = p_content_id AND cm.status = 'visible' AND c.is_active = true AND (c.content_kind = 'news' OR c.show_in_news = true)
...
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.global_announcements WHERE id = p_content_id AND is_active = true AND mention_all = true) THEN RETURN false; END IF;
  INSERT INTO public.global_content_banner_dismissals(content_id,user_id) VALUES (p_content_id,v_user_id) ON CONFLICT (content_id,user_id) DO UPDATE SET dismissed_at = now();
  RETURN true;
END;
$$;

CREATE OR REPLACE FUNCTION public.get_global_content_banners()
RETURNS TABLE (id uuid,title text,message text,type text,content_kind text,show_in_news boolean,published_at timestamptz,action_url text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
  SELECT c.id,c.title,c.message,c.type,c.content_kind,c.show_in_news,c.published_at,
    CASE WHEN c.content_kind = 'news' OR c.show_in_news THEN '/news?item=' || c.id::text ELSE '/announcements?item=' || c.id::text END
  FROM public.global_announcements c
  WHERE auth.uid() IS NOT NULL AND c.is_active = true AND c.mention_all = true
    AND NOT EXISTS (SELECT 1 FROM public.global_content_banner_dismissals d WHERE d.content_id=c.id AND d.user_id=auth.uid())
  ORDER BY c.is_pinned DESC,c.published_at DESC LIMIT 3;
$$;

CREATE OR REPLACE FUNCTION public.admin_get_global_content_comments(p_content_id uuid)
RETURNS TABLE (id uuid,user_id uuid,body text,status text,created_at timestamptz,moderated_at timestamptz,moderation_reason text,author_name text,author_email text,author_avatar text)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
BEGIN
  IF NOT public.can_manage_global_content(auth.uid()) THEN RAISE EXCEPTION 'Not authorized to moderate DRIGHT comments'; END IF;
  RETURN QUERY SELECT cm.id,cm.user_id,cm.body,cm.status,cm.created_at,cm.moderated_at,cm.moderation_reason,COALESCE(NULLIF(u.full_name,''),split_part(u.email,'@',1),'DRIGHT user'),u.email,u.avatar_url
  FROM public.global_content_comments cm LEFT JOIN public.users u ON u.id=cm.user_id WHERE cm.content_id=p_content_id ORDER BY cm.created_at DESC;
END;
$$;

CREATE OR REPLACE FUNCTION public.admin_moderate_global_content_comment(p_comment_id uuid,p_action text,p_reason text DEFAULT NULL)
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE v_action text:=lower(trim(COALESCE(p_action,''))); v_content_id uuid;
BEGIN
  IF NOT public.can_manage_global_content(auth.uid()) THEN RAISE EXCEPTION 'Not authorized to moderate DRIGHT comments'; END IF;
  SELECT content_id INTO v_content_id FROM public.global_content_comments WHERE id=p_comment_id;
  IF v_content_id IS NULL THEN RETURN false; END IF;
  IF v_action='delete' THEN DELETE FROM public.global_content_comments WHERE id=p_comment_id;
  ELSIF v_action='hide' THEN UPDATE public.global_content_comments SET status='hidden',moderated_by=auth.uid(),moderated_at=now(),moderation_reason=NULLIF(trim(COALESCE(p_reason,'')),''),updated_at=now() WHERE id=p_comment_id;
  ELSIF v_action IN ('restore','show') THEN UPDATE public.global_content_comments SET status='visible',moderated_by=auth.uid(),moderated_at=now(),moderation_reason=NULLIF(trim(COALESCE(p_reason,'')),''),updated_at=now() WHERE id=p_comment_id;
  ELSE RAISE EXCEPTION 'Invalid moderation action'; END IF;
  INSERT INTO public.admin_logs(admin_id,action_type,target_id,target_type,details) VALUES (auth.uid(),'news_comment_'||v_action,p_comment_id,'global_content_comment',jsonb_build_object('content_id',v_content_id,'reason',NULLIF(trim(COALESCE(p_reason,'')),'')));
  RETURN true;
END;
$$;
