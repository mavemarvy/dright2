...
create table if not exists public.sales_team_settings (
 id boolean primary key default true check(id), default_member_level integer not null default 3 check(default_member_level between 3 and 5), require_active_team_for_attribution boolean not null default true, allow_product_specific_links boolean not null default true, allow_campaign_links boolean not null default true, max_active_teams_per_member integer not null default 1 check(max_active_teams_per_member between 1 and 20), updated_by uuid references public.users(id) on delete set null, updated_at timestamptz not null default now()
);
insert into public.sales_team_settings(id) values(true) on conflict(id) do nothing;
create unique index if not exists idx_sales_team_one_active_membership on public.sales_team_members(user_id) where status='active';
create or replace function public.sales_team_touch_updated_at() returns trigger language plpgsql as $$ begin new.updated_at=now(); return new; end $$;
