-- Extend the authoritative Paystack conversion boundary with commission_splits.
-- Affiliate conversion remains on its existing path; Sales Team / advertiser / partnership
-- attribution is distributed through distribute_order_commission_splits().
CREATE OR REPLACE FUNCTION public.process_paystack_payment(p_reference text, p_user_id uuid, p_amount numeric, p_purpose text DEFAULT 'wallet_funding'::text, p_reference_id uuid DEFAULT NULL::uuid, p_metadata jsonb DEFAULT '{}'::jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_tx RECORD; v_wallet_id uuid; v_result jsonb; v_order RECORD; v_seller_wallet_id uuid; v_affiliate_wallet_id uuid; v_sale_id uuid;
  v_commission numeric:=0; v_seller_earnings numeric:=0; v_order_final numeric:=0; v_attributed_split jsonb:='{}'::jsonb;
BEGIN
  ...
  IF v_tx.status='success' AND v_tx.processed_at IS NOT NULL THEN RETURN jsonb_build_object('success',true,'message','Already processed','idempotent',true); END IF;
  ...
  ELSIF p_purpose IN ('product_purchase','escrow') THEN
    ...
    IF v_order.status<>'COMPLETED' THEN
      ...
      END IF;
      IF COALESCE(v_order.source_type,'') IN ('sales_team','advertiser','pro_advertiser','super_advertiser','partnership') THEN
