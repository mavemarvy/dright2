...
RETURNS TABLE(
  id uuid, profile_type text, questionnaire_id uuid, questionnaire_version integer,
  status text, submitted_at timestamptz, reviewed_at timestamptz,
  user_visible_reason text, created_at timestamptz, updated_at timestamptz
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp
AS $$
  SELECT s.id,s.profile_type,s.questionnaire_id,s.questionnaire_version,s.status,
         s.submitted_at,s.reviewed_at,s.user_visible_reason,s.created_at,s.updated_at
...
RETURNS TABLE(
  id uuid, submission_id uuid, action text, previous_status text, new_status text,
  user_visible_reason text, created_at timestamptz
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp
AS $$
...
RETURNS TABLE(
  id uuid, profile_id uuid, user_id uuid, status text, provider_id uuid,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp
AS $$
  SELECT s.id,s.profile_id,s.user_id,s.status,s.provider_id,s.rejection_reason,
         s.submitted_at,s.reviewed_at,s.version,s.created_at,s.updated_at
...
RETURNS TABLE(
  id uuid, submission_id uuid, user_id uuid, doc_type text, doc_url text,
  storage_bucket text, storage_path text, doc_name text, doc_mime_type text,
  doc_size_bytes bigint, status text, issuing_country text, document_number_last4 text,
  document_side text, expires_at timestamptz, replaced_by uuid, user_visible_reason text,
  reviewed_at timestamptz, review_source text, metadata jsonb, version integer,
  created_at timestamptz, updated_at timestamptz
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp
AS $$
  SELECT d.id,d.submission_id,d.user_id,d.doc_type,d.doc_url,d.storage_bucket,d.storage_path,
         d.doc_name,d.doc_mime_type,d.doc_size_bytes,d.status,d.issuing_country,
         d.document_number_last4,d.document_side,d.expires_at,d.replaced_by,
         d.user_visible_reason,d.reviewed_at,d.review_source,d.metadata,d.version,
         d.created_at,d.updated_at
  FROM public.kyc_documents d
  WHERE d.user_id=auth.uid() AND d.is_deleted=false
    AND (p_submission_id IS NULL OR d.submission_id=p_submission_id)
...
RETURNS TABLE(
  id uuid, submission_id uuid, action text, user_visible_reason text, created_at timestamptz
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp
AS $$
