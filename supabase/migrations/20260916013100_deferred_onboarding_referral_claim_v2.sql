-- Deferred onboarding + durable referral attribution (compatible v2).
...
BEGIN
  ...
  LOOP
    ...
    IF v_attempt>20 THEN RAISE EXCEPTION 'could not allocate referral code'; END IF;
  END LOOP;
...
UPDATE public.users SET referral_code=NULL
...
BEGIN
  ...
  IF v_parent2 IS NOT NULL AND v_parent2<>p_user_id AND v_parent2<>v_referrer THEN
    ...
    IF v_parent3 IS NOT NULL AND v_parent3<>p_user_id AND v_parent3<>v_referrer AND v_parent3<>v_parent2 THEN
      INSERT INTO public.referral_relationships(referrer_id,referred_id,level)
...
BEGIN
  ...
  SELECT nullif(btrim(raw_user_meta_data->>'signup_referral_code'),'') INTO v_code
...
BEGIN
  ...
  IF v_code IS NOT NULL THEN PERFORM public.apply_signup_referral(NEW.id,v_code); END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  LOOP
    ...
    IF v_condition_key IS NOT NULL AND q.conditional_rules ? 'equals' THEN
      ...
      IF v_condition_actual IS DISTINCT FROM v_condition_expected THEN CONTINUE; END IF;
    END IF;
  ...
  WITH active_defs AS (
    ...
    SELECT ad.id,
      (SELECT qs.status
       ...
       ORDER BY qs.created_at DESC LIMIT 1) status
    FROM active_defs ad
  )
  SELECT count(*) FILTER(WHERE status IS NULL OR status NOT IN('submitted','under_review','approved'))
  INTO v_incomplete FROM latest;
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'authentication required'; END IF;
  SELECT * INTO v_q FROM public.questionnaire_definitions WHERE id=p_questionnaire_id AND is_active=true;
  ...
  ORDER BY qs.created_at DESC LIMIT 1 FOR UPDATE;
  ...
  IF v_submission IS NULL THEN
    ...
    VALUES(v_uid,v_q.applicable_profile_type,v_q.id,v_q.version,v_status,CASE WHEN p_submit THEN now() ELSE NULL END)
  ...
  SELECT v_submission,qq.id,qq.question_key,
         ...
         jsonb_build_object(
           'question_key',qq.question_key,'label',qq.label,'description',qq.description,
           'answer_type',qq.answer_type,'options',qq.options,'required',qq.is_required,
           'sort_order',qq.sort_order,'conditional_rules',qq.conditional_rules,
           'validation_rules',qq.validation_rules,'questionnaire_key',v_q.questionnaire_key,
...
BEGIN
  ...
  VALUES(
    v_uid,upper(p_country_iso2),p_country_calling_code,p_date_of_birth,p_intended_profiles,
    coalesce(p_interests,'{}'::text[]),'in_progress',NULL
  )
...
BEGIN
  ...
  SELECT * INTO v_kyc FROM public.kyc_profiles WHERE user_id=v_uid AND is_deleted=false ORDER BY updated_at DESC LIMIT 1;
...
CREATE OR REPLACE FUNCTION public.recalculate_user_eligibility(
  p_user_id uuid,
  p_profile_type text,
  p_action_key text DEFAULT 'general'::text
)
...
DECLARE
  ...
  v_min_age integer;
BEGIN
  IF auth.uid() IS DISTINCT FROM p_user_id AND NOT public.has_dright_permission('eligibility','recalculate') THEN
  ...
  WHERE user_id=p_user_id
    ...
    AND (user_type=p_profile_type OR p_profile_type=ANY(applicable_profile_types))
  ORDER BY updated_at DESC LIMIT 1;
  SELECT qs.status INTO v_q_status
  FROM public.questionnaire_submissions qs
  WHERE qs.user_id=p_user_id AND qs.profile_type=p_profile_type
  ...
  IF v_private.date_of_birth IS NULL THEN
    ...
    v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object('key','age','ok',false,'message','Date of birth is required'));
  ELSE
    v_min_age:=coalesce(v_age_rule.minimum_age,0);
    IF v_age_rule.profile_type IS NOT NULL AND jsonb_typeof(v_age_rule.country_overrides->v_private.country_iso2)='number' THEN
      v_min_age:=(v_age_rule.country_overrides->>v_private.country_iso2)::integer;
    END IF;
    IF public.dright_age_on(v_private.date_of_birth)<v_min_age THEN
      v_ok:=false;
      v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object('key','age','ok',false,'message','Minimum age requirement is not satisfied'));
    ELSE
  ...
  IF v_q_status IS NULL OR v_q_status NOT IN('submitted','under_review','approved') THEN
    ...
    v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object(
      'key','questionnaire','ok',false,
      'message','Required questionnaire is incomplete or needs changes',
      'status',coalesce(v_q_status,'not_started')
    ));
  ELSE
  ...
  IF coalesce(v_kyc_rule.is_required,false) THEN
    ...
    ELSE
      v_ok:=false;v_more:=true;
      v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object('key','kyc','ok',false,'message','Required KYC is not verified','status',coalesce(v_kyc.status,'not_started')));
    END IF;
  ELSE
  ...
  IF NOT v_email_verified THEN
    v_ok:=false;v_more:=true;
    v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object('key','email','ok',false,'message','Email verification required'));
  ELSE
  ...
  IF v_user.account_status<>'ACTIVE' THEN
    v_ok:=false;
    v_reasons:=v_reasons||jsonb_build_array(jsonb_build_object('key','account','ok',false,'message','Account has an active restriction','status',v_user.account_status));
  ELSE
