...
SET applicable_profile_types=ARRAY[user_type]
WHERE cardinality(applicable_profile_types)=0;
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE d.submission_id=p_submission_id AND d.doc_type=req AND d.is_deleted=false AND d.status='verified'
      AND (d.expires_at IS NULL OR d.expires_at>now())
  );
  SELECT count(*) INTO v_problem FROM public.kyc_documents d
  WHERE d.submission_id=p_submission_id AND d.doc_type=ANY(v_required) AND d.is_deleted=false
    AND d.status IN ('rejected','needs_resubmission','unreadable','expired','suspected_fraud');

  IF v_problem>0 THEN v_new:='more_info_required';
