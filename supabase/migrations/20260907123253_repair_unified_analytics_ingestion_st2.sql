...
-- Preserve legacy product_views/listing_events/search_history while making analytics_events authoritative.

create unique index if not exists idx_analytics_sessions_session_id_unique
  on public.analytics_sessions(session_id);

-- The unified event table must not be directly client-writable. Ingestion is via track-event -> RPC.
...
begin
  ...
  if p_event_type is null or btrim(p_event_type) = '' or length(p_event_type) > 80 then
    raise exception 'invalid event_type';
  end if;

  if p_session_id is not null and length(p_session_id) > 160 then
    raise exception 'invalid session_id';
  end if;

  -- Deduplicate passive exposure/view events only. Never blanket-deduplicate purchases,
  -- payments, conversions, clicks, checkout actions, referrals, or other high-value actions.
  v_cooldown := case
  ...
  if v_cooldown is not null and p_entity_id is not null then
    ...
    where ae.entity_id = p_entity_id
      ...
      and (
        (v_viewer_id is not null and ae.viewer_id = v_viewer_id)
  ...
  -- Maintain the existing analytics_sessions table from the authoritative event stream.
  if p_session_id is not null and btrim(p_session_id) <> '' then
    insert into public.analytics_sessions (
      session_id,user_id,device_type,os,browser,language,timezone,country,
      entry_page,exit_page,page_views,events_count,duration_seconds,is_bounce,started_at,ended_at
    ) values (
  ...
  -- Legacy compatibility: preserve product_views and the existing products.view_count counter.
  if p_event_type = 'product_view' and p_entity_type = 'product' and p_entity_id is not null then
    insert into public.product_views(product_id,user_id,viewed_at)
    values (p_entity_id,v_viewer_id,v_created_at);
  ...
  -- Legacy compatibility for listing intelligence. Avoid duplicate mirrors if two UI helpers
  -- report the same open/action within the same render cycle.
  ...
  if v_legacy_event_type is not null and p_entity_id is not null and p_entity_type in ('product','service','job','course','digital_download') then
    if not exists (
      ...
      where le.listing_id = p_entity_id
        and le.event_type = v_legacy_event_type
        and le.created_at >= now() - interval '5 seconds'
        and (
          (v_viewer_id is not null and le.user_id = v_viewer_id)
  ...
  -- Preserve the existing search_history dataset while routing searches through the unified pipeline.
  if p_event_type = 'search' and nullif(btrim(coalesce(p_metadata->>'query','')),'') is not null then
    ...
    values (
      v_viewer_id,
      left(p_metadata->>'query',500),
      nullif(left(coalesce(p_metadata->>'category',''),200),''),
...
-- Keep the RPC callable through PostgREST with the caller JWT so auth.uid() is authoritative.
