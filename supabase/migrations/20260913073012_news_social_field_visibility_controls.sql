...
BEGIN
  ...
  SET hide_view_count = COALESCE(p_hide_view_count, false),
      hide_reaction_count = COALESCE(p_hide_reaction_count, false),
      updated_at = now()
  ...
  VALUES (auth.uid(), 'global_content_display_settings_updated', p_content_id, 'global_content',
    jsonb_build_object('hide_view_count', COALESCE(p_hide_view_count,false), 'hide_reaction_count', COALESCE(p_hide_reaction_count,false)));
  RETURN true;
END;
$$;
...
-- Keep view counting authoritative and unique per signed-in viewer. This is a view count,
-- not a raw click counter, so reopening the same post does not inflate it.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id=p_content_id AND is_active=true AND (content_kind='news' OR show_in_news=true)
  ) THEN RAISE EXCEPTION 'Content not available'; END IF;
  ...
  UPDATE public.global_announcements ga
  SET view_count = (SELECT count(*) FROM public.global_content_views v WHERE v.content_id=ga.id)
  WHERE ga.id=p_content_id
  RETURNING ga.view_count INTO v_count;
