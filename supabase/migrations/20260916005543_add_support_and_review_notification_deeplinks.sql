...
begin
  ...
  if cardinality(v_recipients) > 0 then
    ...
    values(
      ...
      jsonb_build_object('ticket_id',new.id,'ticket_number',new.ticket_number,'subject',new.subject,'channel',new.channel,'action_url','/admin/tickets'),
      true
    );
  end if;
  return new;
...
begin
  ...
  if new.author_role = 'admin' then
    ...
    if not new.is_internal then
      insert into public.notifications(
        ...
        'admin_notice',new.ticket_id,'admin','high',false,
  ...
  if cardinality(v_recipients) > 0 and not new.is_internal then
    ...
    values(
      ...
      jsonb_build_object('ticket_id',new.ticket_id,'ticket_number',v_ticket.ticket_number,'reply_id',new.id,'channel',new.channel,'action_url',case when new.author_role='admin' then '/help#support' else '/admin/tickets' end),true
    );
  end if;
...
begin
  ...
  insert into public.notifications(
    ...
    'admin','normal',false,
    jsonb_build_object('event_type','customer_care_in_app_message','contact_id',new.contact_id,'contact_log_id',new.id,'support_ticket_id',v_contact.support_ticket_id,'staff_name',coalesce(v_staff_name,'DRIGHT Support'),'action_url','/help#support'),
